...
<table align="center" border="1" width="70%">
    ...
    <tr>
        ...
        <td>
         ...
         ArrayList<String> phones = person.getPhones();
         for(int i = 0; i < phones.size(); i++) {
         	out.write(phones.get(i));%>
         	<a href="<%=request.getContextPath()%>/?action=edit&id=<%=person.getId()%>&phone_id=<%new String(i)%>">Редактировать</a>
         	<textarea> </textarea>
         	<a href="<%=request.getContextPath()%>/?action=delete&id=<%=person.getId()%>&phone_id=<%new String(i)%>">Удалить</a>
         	<%
         }
         %></textarea>
         <a href="<%=request.getContextPath()%>/?action=add&id=<%=person.getId()%>">Добавить</a>
        </td>
    </tr>
    <% } %>
    <tr>
