...
public class Phonebook {
	...
	public boolean addPhone(String id, String number) {
		String query = "INSERT INTO `phone` (`owner`, `number`) VALUES ('" + id +"', '" + number + "')";
		Integer affected_rows = this.db.changeDBData(query);
		// Если обновление прошло успешно...
		if (affected_rows > 0) {
			return true;
	...
	public boolean editPhone(String id, String number, String phone_id) {
		 Integer id_filtered = Integer.parseInt(phone_id);
		 String query= "UPDATE `person` SET `number` = " + number + "' WHERE `owner` = " +
				 		id + "' AND `id` =" + id_filtered;
		 Integer affected_rows = this.db.changeDBData(query);
		 if (affected_rows > 0) {
		 	return true;
		 } else {
	...
	public boolean deletePhone(String id, String number_id) {
